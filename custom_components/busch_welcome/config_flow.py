...
class BuschWelcomeConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def _check_unique(self, gateway_uuid: str) -> ConfigFlowResult | None:
        ...
        for entry in self._async_current_entries():
            if entry.data.get(CONF_HOST) == self._gateway_ip:
    ...
    async def async_step_user(
        ...
        if user_input is not None:
            ...
            self._gateway_ip = user_input[CONF_HOST].strip()
            self._gateway_password = user_input[CONF_GATEWAY_PASSWORD]

            reachable = await self.hass.async_add_executor_job(
                _gateway_reachable, self._gateway_ip, 80 # check standard web port
            )
            if not reachable:
    ...
    def _do_pairing_setup(self) -> None:
        ...
        self._gateway_name = gw_info.get("portalname") or "Busch Welcome Gateway"
    ...
    async def async_step_confirm(
        ...
        if user_input is not None:
            return self.async_create_entry(
                title=f"Busch-Jaeger Welcome ({self._gateway_name})",
                data={
                    CONF_HOST: self._gateway_ip,
